...
namespace MTConnectSharp
{
    ...
    public class Component : MTCItemBase, IComponent
    {
        ...
        internal Component(XElement xmlComponent)
        {
            ...
            Id = xmlComponent.GetAttribute("id");
            Name = xmlComponent.GetAttribute("name");
            if (string.IsNullOrEmpty(Name))
                Name = Id;
            NativeName = xmlComponent.GetAttribute( "nativeName");

            _dataItems = new ObservableCollection<DataItem>(xmlComponent.GetDataItems());
            DataItems = new ReadOnlyObservableCollection<DataItem>(_dataItems);

            _components = new ObservableCollection<Component>(xmlComponent.GetComponents());
